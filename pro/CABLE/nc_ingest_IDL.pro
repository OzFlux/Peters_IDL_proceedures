...
;NCFileName=InFilePath+BaseName+'.nc'
NCFilePath=getenv('NCFilePath')
if strlen(NCFilePath) eq 0 then begin
 if strlen(InFilePath) eq 0 then begin
  NCFileName=Dialog_PickFile(TITLE='Save output file as',GET_PATH=NCFilePath)
 endif else begin
  NCFileName=Dialog_PickFile(TITLE='Save output file as',PATH=InFilePath,GET_PATH=NCFilePath)
 endelse
endif else begin
 NCFileName=Dialog_PickFile(TITLE='Save output file as',PATH=NCFilePath,GET_PATH=NCFilePath)
endelse
if (strlen(NCFileName) eq 0) then goto, finish
...
Line=''
readf, InLUN, Line
Header=strsplit(Line,',',/extract,count=NFields)
nTimeSteps=nTimeSteps-1

Data=fltarr(NFields,nTimeSteps)
readf, InLUN, Data
free_lun, InLUN
...
;   VarList=VarList+'RH,'
  ...
  else: print,'Variable ',strcompress(Header[i],/remove_all),' found in file but not treated'
  endcase
 endfor
...
ncid=ncdf_create(NCFileName,/clobber)
...
DimID=make_array(nc.NDim,/integer)
...
VarID=make_array(nc.NVar,/integer)
